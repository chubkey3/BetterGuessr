...
const Room = () => {
  ...
  const socket = useMemo(
    () => io(process.env.PROD == "true" ? "https://api.chubkey.com" : "http://localhost:13242"),
    []
  );
      ...
      {!started ? (
        user ? (
          <div className="lobby">
            ...
            {message === "Connected" ? (
              <h1 className="good_message">Status: {message}</h1>
            ) : (
              <h1 className="bad_message">Status: {message}</h1>
            )}
            ...
            <button onClick={switchTeams} className={"but switch-teams-button"}>
              switch team
            </button>
            <button
              onClick={startRoom}
              className={team1?.length! > 0 && team2?.length! > 0 ? "but" : "disabled-button"}
            >
              Start
            </button>
          </div>
          ...
          <div className="main-wrapper" style={styles}>
            ...
            {roundEnd && (
              <FullscreenMap
                markers={markers}
                center={center}
                team1_health={health?.team1}
                team2_health={health?.team2}
                team1_distance={team1distance}
                team2_distance={team2distance}
                countdown={countdown}
                round={round}
                multiplier={multiplier}
                team={user && team1?.includes(user) ? "team1" : "team2"}
              />
            )}
            {win && (
              <div className="win-overlay">
                <text>{win}</text>
                <button className="but" onClick={() => window.location.reload()}>
                  play again
                </button>
              </div>
            )}
